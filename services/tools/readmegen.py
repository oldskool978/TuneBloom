...
def generate_services_readme() -> None:
    bt = chr(96) * 3
    sbt = chr(96)
    
    tools_dir = Path(__file__).resolve().parent
    services_dir = tools_dir.parent
    readme_path = services_dir / "README.md"

    content = f"""# TuneBloom Unified Services & Compute Daemon Routing Layer
...
* **Hermetic Hardware Memory Reclamation**: Automatic invocation of {sbt}gc.collect(){sbt}, {sbt}torch.cuda.empty_cache(){sbt}, and {sbt}torch.cuda.ipc_collect(){sbt} between sequential inference stages ensures deterministic VRAM ceilings.
...
* **Isolated Default Starter Asset**: Excludes reserved artwork ({sbt}default.jpg{sbt}) from dynamic hash roulette assignment pools to ensure the permanent showcase track remains anchored.
...
| :--- | :--- | :--- |
| **GET** | {sbt}/auth/challenge{sbt} | Issues dynamic PoW challenge with difficulty target and HMAC signature |
| **POST** | {sbt}/auth/login{sbt} | Authenticates user handle, seeds quota, returns token and track history |
| **GET** | {sbt}/themes/registry{sbt} | Serves registered theme palettes, ambient animations, and easter eggs |
| **GET** | {sbt}/health{sbt} | Provides engine availability and active compute device status |
| **POST** | {sbt}/synthesize{sbt} | Validates PoW solution, admits job to compute queue, returns job status |
| **GET** | {sbt}/jobs/{{job_id}}{sbt} | Polls real-time progress percentage, stage description, and queue position |
| **GET** | {sbt}/audio/{{job_id}}{sbt} | Direct download stream for temporary artifact bitstreams |
| **GET** | {sbt}/audio/stream/{{user_slug}}/{{filename}}{sbt} | Serves persisted master audio streams from user discography vault |
...
"""
    ...
    print(f"[+] Successfully generated services documentation: {readme_path}")
