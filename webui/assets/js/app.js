...
function isTrackCompleted(track) {
  return Boolean(track && (track.status === "COMPLETED" || Boolean(track.audio_url)));
}
    ...
    defaultTrack = {
      ...
      recipe: {
        ...
        telemetry: {
          ...
          stage1_late_vocal_solver: "ipndm",
          stage1_handoff_threshold: 0.3257,
          stage1_num_inference_steps: 42,
          stage1_early_instrumental_cfg: 1.78,
    ...
    if (authData) {
      ...
      if (Array.isArray(authData.tracks) && storage) {
        for (let i = 0; i < authData.tracks.length; i++) {
          ...
          const isComp = Boolean(remoteTrack.status === "COMPLETED" || resolvedAudioUrl || remoteTrack.recipe);
          await storage.saveTrack({
            ...remoteTrack,
            status: isComp ? "COMPLETED" : (remoteTrack.status || "DRAFT"),
...
async function selectTrackById(trackId, autoMountPlayer = true) {
  ...
  if (isCompleted) {
    AppState.activeTrackCleanRecipe = computeCanonicalRecipe(track.title, track.recipe || track.working_draft);
  } else {
    AppState.activeTrackCleanRecipe = null;
  }
  ...
  syncTimeout = setTimeout(async () => {
    ...
    if (isCompleted) {
      track.fork_draft = {
        ...
        top_k: track.working_draft?.top_k,
        top_k_layers: track.working_draft?.top_k_layers,
        ar_guidance_scale: track.working_draft?.ar_guidance_scale,
        early_instrumental_solver: track.working_draft?.early_instrumental_solver,
        late_instrumental_solver: track.working_draft?.late_instrumental_solver,
        early_vocal_solver: track.working_draft?.early_vocal_solver,
        late_vocal_solver: track.working_draft?.late_vocal_solver,
        handoff_threshold: track.working_draft?.handoff_threshold,
        num_inference_steps: track.working_draft?.num_inference_steps,
        early_instrumental_cfg: track.working_draft?.early_instrumental_cfg,
...
async function handleGenerateSubmit(e) {
  ...
  if (isFork) {
    ...
    if (parentCover) usedSet.add(parentCover);
  ...
  if (isFork) {
    if (currentTrack && currentTrack.recipe) {
      ...
      currentTrack.working_draft = {
        ...
        top_k: currentTrack.working_draft?.top_k,
        top_k_layers: currentTrack.working_draft?.top_k_layers,
        ar_guidance_scale: currentTrack.working_draft?.ar_guidance_scale,
        early_instrumental_solver: currentTrack.working_draft?.early_instrumental_solver,
        late_instrumental_solver: currentTrack.working_draft?.late_instrumental_solver,
        early_vocal_solver: currentTrack.working_draft?.early_vocal_solver,
        late_vocal_solver: currentTrack.working_draft?.late_vocal_solver,
        handoff_threshold: currentTrack.working_draft?.handoff_threshold,
        num_inference_steps: currentTrack.working_draft?.num_inference_steps,
        early_instrumental_cfg: currentTrack.working_draft?.early_instrumental_cfg,
  ...
  try {
    ...
    const payload = {
      ...
      ...(currentTrack?.working_draft?.top_k_layers ? { top_k_layers: currentTrack.working_draft.top_k_layers } : {}),
      ...(currentTrack?.working_draft?.ar_guidance_scale !== undefined ? { ar_guidance_scale: currentTrack.working_draft.ar_guidance_scale } : {}),
      ...(currentTrack?.working_draft?.early_instrumental_solver ? { early_instrumental_solver: currentTrack.working_draft.early_instrumental_solver } : {}),
      ...(currentTrack?.working_draft?.late_instrumental_solver ? { late_instrumental_solver: currentTrack.working_draft.late_instrumental_solver } : {}),
      ...(currentTrack?.working_draft?.early_vocal_solver ? { early_vocal_solver: currentTrack.working_draft.early_vocal_solver } : {}),
      ...(currentTrack?.working_draft?.late_vocal_solver ? { late_vocal_solver: currentTrack.working_draft.late_vocal_solver } : {}),
      ...(currentTrack?.working_draft?.handoff_threshold !== undefined ? { handoff_threshold: currentTrack.working_draft.handoff_threshold } : {}),
      ...(currentTrack?.working_draft?.num_inference_steps !== undefined ? { num_inference_steps: currentTrack.working_draft.num_inference_steps } : {}),
      ...(currentTrack?.working_draft?.early_instrumental_cfg !== undefined ? { early_instrumental_cfg: currentTrack.working_draft.early_instrumental_cfg } : {}),
    ...
    try {
      ...
      if (userRecord) {
        ...
        for (const t of AppState.tracks) {
          if (isTrackCompleted(t) && t.status !== "COMPLETED") {
            t.status = "COMPLETED";
            await window.clientStorage.saveTrack(t);
          }
        }
                ...
                if (Array.isArray(data.tracks)) {
                  ...
                  for (let i = 0; i < data.tracks.length; i++) {
                    ...
                    if (existingIdx === -1) {
                      ...
                      if (AppState.tracks[existingIdx].status !== "COMPLETED" && trackObj.status === "COMPLETED") {
                        AppState.tracks[existingIdx] = trackObj;
                        await window.clientStorage.saveTrack(trackObj);
                        changed = true;
                      } else if (!AppState.tracks[existingIdx].audio_url && trackObj.audio_url) {
                        AppState.tracks[existingIdx].audio_url = trackObj.audio_url;
                        AppState.tracks[existingIdx].status = "COMPLETED";
                        await window.clientStorage.saveTrack(AppState.tracks[existingIdx]);
                        changed = true;
                      }
                    }
                  }
...
window.isTrackCompleted = isTrackCompleted;
