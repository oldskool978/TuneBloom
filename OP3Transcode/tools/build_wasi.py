...
def stage_builtins(wasi_build: Path) -> None:
    ...
    if not builtins and (WASI_BUILTINS_DIR / "libclang_rt.builtins-wasm32.a").exists():
        builtins = [WASI_BUILTINS_DIR / "libclang_rt.builtins-wasm32.a"]
...
def main() -> None:
    ...
    builtin_candidate = WASI_BUILTINS_DIR / "libclang_rt.builtins-wasm32.a"
    if not builtin_candidate.exists():
        builtin_candidate = WASI_BUILTINS_DIR / "libclang_rt.builtins.a"
    ...
    if builtin_candidate.exists():
        b_path_clean = sanitize_path(builtin_candidate)
        cmake_cmd.append(f"-DBUILTINS_LIB={b_path_clean}")
        cmake_cmd.append(f"-DWASI_SDK_BUILTINS={b_path_clean}")
