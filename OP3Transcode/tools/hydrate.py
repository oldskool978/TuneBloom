...
WASI_BUILTINS_DIR = TOOLCHAIN_DIR / "wasi-builtins"
...
URL_LAME = "https://downloads.sourceforge.net/project/lame/lame/3.100/lame-3.100.tar.gz"
URL_TAILWIND = "https://cdn.tailwindcss.com"

URL_WASI_BUILTINS_FALLBACKS = [
    "https://github.com/WebAssembly/wasi-sdk/releases/download/wasi-sdk-24/libclang_rt.builtins-wasm32-wasi-24.0.tar.gz",
...
def hydrate_python() -> None:
    if HERMETIC_PYTHON_EXE.exists():
        print(f"  [+] Hermetic Python runtime verified: {HERMETIC_PYTHON_EXE.name}")
        return
...
def hydrate_toolchains() -> None:
    ...
    if IS_WIN:
        if not GIT_CMD_EXE.exists() and git_url:
            print("[*] Hydrating MinGit portable...")
            ...
            print("  [+] MinGit toolchain verified.")

    if not CMAKE_EXE.exists():
        print("[*] Hydrating CMake build engine...")
        ...
        print("  [+] CMake build engine verified.")

    if not NINJA_EXE.exists():
        print("[*] Hydrating Ninja executor...")
        ...
        print("  [+] Ninja executor verified.")

    if not (CLANG_CL_EXE if IS_WIN else CLANG_EXE).exists():
        print("[*] Hydrating LLVM/Clang compiler toolchain...")
        ...
        print("  [+] LLVM/Clang compiler toolchain verified.")

    if IS_WIN:
        if not NASM_EXE.exists() and nasm_url:
            print("[*] Hydrating NASM assembler...")
            ...
            print("  [+] NASM assembler verified.")

def fetch_msvc_manifest_catalog() -> dict:
    for endpoint in VS_CHANNEL_ENDPOINTS:
        try:
            print(f"[*] Resolving MSVC Visual Studio manifest: {endpoint}")
...
def hydrate_winsdk() -> None:
    ...
    if (msvc_inc_target / "vcruntime.h").exists() and version_marker.exists():
        print("  [+] Windows SDK & MSVC CRT fakeroot verified intact.")
        return
    ...
    print("  [+] Windows SDK & MSVC CRT fakeroot staged successfully.")

def hydrate_wasi_builtins() -> None:
    WASI_BUILTINS_DIR.mkdir(parents=True, exist_ok=True)
    target_lib = WASI_BUILTINS_DIR / "libclang_rt.builtins-wasm32.a"
    if target_lib.exists() and target_lib.stat().st_size > 0:
        print("  [+] WebAssembly compiler-rt builtins verified.")
        return

    print("[*] Hydrating WebAssembly compiler-rt builtins archive...")
    wasi_sdk_rel = fetch_github_latest_release("WebAssembly/wasi-sdk")
    wasi_builtins_url = ""
    if wasi_sdk_rel:
        for asset in wasi_sdk_rel.get("assets", []):
            name = asset.get("name", "").lower()
            if "builtins" in name and ("wasm32" in name or "wasi" in name) and name.endswith(".tar.gz"):
                wasi_builtins_url = asset.get("browser_download_url")
                break

    if not wasi_builtins_url:
        for fallback_candidate in URL_WASI_BUILTINS_FALLBACKS:
            if probe_url(fallback_candidate):
                wasi_builtins_url = fallback_candidate
                break

    if not wasi_builtins_url:
        wasi_builtins_url = URL_WASI_BUILTINS_FALLBACKS[0]

    archive = TMP_DIR / "wasi_builtins.tar.gz"
    fetch_payload(wasi_builtins_url, archive)
    temp_ext = TMP_DIR / "wasi_builtins_ext"
    unpack_archive(archive, temp_ext, flatten_single_dir=False)

    found_libs = list(temp_ext.rglob("*clang_rt*.a")) + list(temp_ext.rglob("*builtins*.a"))
    if not found_libs:
        raise RuntimeError("Failed to locate compiler-rt builtins library inside archive.")

    shutil.copy2(to_unc_path(found_libs[0]), to_unc_path(target_lib))
    shutil.copy2(to_unc_path(found_libs[0]), to_unc_path(WASI_BUILTINS_DIR / "libclang_rt.builtins.a"))
    ...
    for triple in ["wasm32-unknown-wasip1", "wasm32-wasip1", "wasm32-unknown-wasi", "wasi"]:
        ...
        shutil.copy2(to_unc_path(target_lib), to_unc_path(td / "libclang_rt.builtins.a"))
        shutil.copy2(to_unc_path(target_lib), to_unc_path(td / "libclang_rt.builtins-wasm32.a"))

    safe_rmtree(temp_ext)
    archive.unlink(missing_ok=True)
    print("  [+] Staged WebAssembly compiler-rt builtins successfully.")
...
def hydrate_libraries() -> None:
    ...
    if not (LIBRARY_DIR / "opus" / "src" / "opus_decoder.c").exists():
        ...
        print("  [+] Opus decoder source repository verified.")
    ...
    if not (LIBRARY_DIR / "libogg" / "src" / "framing.c").exists():
        ...
        print("  [+] LibOgg bitstream source repository verified.")
    ...
    if not (LIBRARY_DIR / "wasi-libc" / "Makefile").exists() and not (LIBRARY_DIR / "wasi-libc" / "CMakeLists.txt").exists():
        ...
        print("  [+] WASI Libc source repository verified.")

    if not (LIBRARY_DIR / "lame" / "libmp3lame" / "lame.c").exists():
        archive = TMP_DIR / "lame.tar.gz"
        fetch_payload(URL_LAME, archive)
        unpack_archive(archive, LIBRARY_DIR / "lame", flatten_single_dir=True)
        archive.unlink(missing_ok=True)
    else:
        print("  [+] LAME encoder source repository verified.")

    tailwind_dest = LIBS_DIR / "tailwind.js"
    if not tailwind_dest.exists():
        fetch_payload(URL_TAILWIND, tailwind_dest)
    else:
        print("  [+] Testbench UI dependencies verified.")

def main() -> None:
    TMP_DIR.mkdir(parents=True, exist_ok=True)
    print("================================================================================")
    print("           TUNEBLOOM OP3TRANSCODE HERMETIC HYDRATION SUITE                      ")
    print("================================================================================")
    hydrate_python()
    hydrate_toolchains()
    hydrate_winsdk()
    hydrate_wasi_builtins()
    hydrate_libraries()
    print("--------------------------------------------------------------------------------")
